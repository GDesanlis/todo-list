...
class Tasks {
    ...
    constructor(lib, server, options, next) {
        ...
        internals._exposePluginMethods(server, internals._lib, internals._pluginCfg.expose.tasks, 'Tasks');
...
internals._exposePluginMethods = (server, obj, config, pluginName) => {
    ...
    server.log(['debug'], 'START < Tasks.internals._exposePluginMethods >');
    ...
    if (config && config instanceof Array) {
        ...
        config.forEach((method) => {
            ...
            if (obj[method]) {
                server.log(['debug'], '< Tasks.internals._exposePluginMethods > Method `' + method + '` exists for plugin `' + pluginName + '`');

                ObjToExpose[method] = obj[method];
            }
            else {
                server.log(['debug'], '< Tasks.internals._exposePluginMethods > Method `' + method + '` doesn\'t exist for plugin `' + pluginName + '`');
            }
        });
        ...
        ['server'].forEach((method) => {
            ...
            if (!ObjToExpose[method]) {
                ...
                server.log(['debug'], '< Tasks.internals._exposePluginMethods > Method `' + method + '` is already exposed for plugin `' + pluginName + '`');
            }
        });

        // check if object has some methods to expose these
        if (Object.keys(ObjToExpose).length > 0) {
            server.log(['debug'], '< Tasks.internals._exposePluginMethods > Plugin `' + pluginName + '` has some methods to be exposed => ' +
                JSON.stringify(Object.keys(ObjToExpose)));

            Object.keys(ObjToExpose).forEach((key) => server.expose(key, ObjToExpose[key]));
        }
        else {
            server.log(['debug'], '< Tasks.internals._exposePluginMethods > Plugin `' + pluginName + '` has no methods to be exposed');
        }
    }
    else {
        server.log(['debug'], '< Tasks.internals._exposePluginMethods > Plugin `' + pluginName + '` has no methods to be exposed');
    ...
    server.log(['debug'], 'END < Tasks.internals._exposePluginMethods >');
};
...
internals._afterUtils = (server, next) => {
    ...
    server.log(['debug'], 'START < Tasks.internals._afterUtils >');

    // store plugin object
    Object.assign(internals._lib, { utils: server.plugins.utils });

    server.log(['debug'], '< Tasks.internals._afterUtils > Store plugin in Lib');

    // set schema in options
    Object.assign(internals._options, { schema: internals._lib.utils.schema });

    server.log(['debug'], '< Tasks.internals._afterUtils > Store schema in options for routes');

    // load routes
    internals._registerRoutes(server, internals._lib, internals._options, internals._routeDir);

    server.log(['debug'], 'END < Tasks.internals._afterUtils >');
...
internals._afterDb = (server, next) => {
    ...
    server.log(['debug'], 'START < Tasks.internals._afterDb >');

    // store plugin object
    Object.assign(internals._lib, { db: server.plugins.db });

    server.log(['debug'], '< Tasks.internals._afterDb > Store plugin in Lib');

    server.log(['debug'], 'END < Tasks.internals._afterDb >');
...
internals._registerRoutes = (server, lib, options, path) => {
    ...
    if (routes) {
        server.log(['debug'], '< Tasks.internals._registerRoutes > New routes folder found in "' + path + '"');

        for (const route in routes) {
            try {
                server.route(new routes[route](lib, options).definition());
                server.log(['debug'], '< Tasks.internals._registerRoutes > Route "' + route + '" in "' + path + '" - OK');
            }
            catch (e) {
                server.log(['debug'], '< Tasks.internals._registerRoutes > "' + route + '" is not a valid route - May be a folder');
        ...
        server.log(['debug'], '< Tasks.internals._registerRoutes > No routes found in "' + path + '" folder');
    }
};
